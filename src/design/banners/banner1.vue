<template>
  ...
  >
    <slide v-for="(banner, index) in setting.slides" :key="index">
      <div :class="`${banner.posicion} slide_content`">
        ...
        <div class="slide_texts" :style="`grid-gap: ${banner.spaces}px;`">
          ...
          <p
            ...
            :style="`font-size: ${banner.descripcionsize}px;`"
          >{{ banner.descripcion }}</p>
          <template v-if="banner.redirect_to">
            <router-link
              v-if="banner.redirect_to.type == 1"
              :to="`/productos/${banner.redirect_to.value}`"
            >
              <button type="button">Ir a la pagina</button>
            </router-link>
            <router-link
              v-else-if="banner.redirect_to.type == 2"
              :to="`/productos?category=${banner.redirect_to.value}`"
            >
              <button type="button">Ir a la pagina</button>
            </router-link>
            <router-link
              v-else-if="banner.redirect_to.type == 3"
              :to="`/productos?search=${banner.redirect_to.value}`"
            >
              <button type="button">Ir a la pagina</button>
            </router-link>
            ...
            <a
              v-else-if="banner.redirect_to.type == 5"
              :href="banner.redirect_to.value"
              target="_blank"
            >
              <button type="button">Ir a la pagina</button>
            </a>
